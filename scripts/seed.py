...
from backend.services.events import log_auth_event
from backend.services.users import create_user, get_user_by_username
...
def seed() -> None:
    ...
    for username, email, home_city in USERS:
        ...
        # 20 normal successful logins over the past 40 days from home. We pass
        # the true historical timestamp so the anomaly engine scores each event
        # against correctly-spaced history (not all at "now").
